    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl p-6 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-[#1A1A1A]">{t('recurring_transactions')}</h2>
        ...
        {!showForm ? (
          <>
            {loading ? (
              ...
              <div className="py-8 text-center">
                <p className="text-[#8FA4C8] text-sm mb-3">{t('error_loading_data') === 'error_loading_data' ? 'Belum ada transaksi berulang' : 'No recurring transactions'}</p>
                <button
                  onClick={() => setShowForm(true)}
                  className="px-4 py-2 rounded-xl bg-[#FF6A00] text-white text-sm font-semibold hover:bg-[#e05e00]"
                >
                  + {t('add')} {t('recurring_transactions')}
                </button>
              </div>
            ) : (
              <>
          ...
          <>
            ...
            <div className="space-y-4 mb-4">
              {/* Type */}
               <div>
                 <label className="text-xs font-semibold text-[#8FA4C8] uppercase mb-2 block">{t('recurring_transaction')}</label>
               ...
               {/* Amount */}
               <div>
                 <label className="text-xs font-semibold text-[#8FA4C8] uppercase mb-1.5 block">{t('amount')}</label>
              ...
              {/* Category */}
              <div>
                <label className="text-xs font-semibold text-[#8FA4C8] uppercase mb-2 block">{t('category')}</label>
                <select
                  value={form.category}
                ...
                >
                  ...
                  {customCategories.map(cat => (
                    <option key={`custom_${cat.id}`} value={`custom_${cat.id}`}>
                      {cat.emoji} {cat.name}
                    </option>
                  ))}
                </select>
              ...
              {/* Note */}
              <div>
                <label className="text-xs font-semibold text-[#8FA4C8] uppercase mb-1.5 block">{t('note_optional')}</label>
                <input
                  type="text"
                  className="w-full border border-[#E2E8F0] rounded-xl px-4 py-3 text-sm text-[#1A1A1A] focus:outline-none focus:ring-2 focus:ring-[#FF6A00] bg-[#F8FAFC]"
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                >
                {saving ? t('saving') : editingId ? t('save_changes') : t('add')}
                </button>
            </div>
          </>
        )}
