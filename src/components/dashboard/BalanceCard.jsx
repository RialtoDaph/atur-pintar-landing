...
export default function BalanceCard({ income, expense, savings, loading, accountTotal }) {
  const { formatCurrency, t } = useAppSettings();
  const balance = accountTotal !== undefined ? accountTotal : (income - expense);
  const balanceLabel = accountTotal !== undefined ? "Total Saldo Rekening" : "Saldo Bulan Ini";
  ...
  return (
    <div data-tour="balance-card" className="bg-gradient-to-br from-[#1a1a1a] via-[#161616] to-[#111] rounded-2xl p-5 border border-[#2a2a2a] shadow-2xl" style={{boxShadow: '0 8px 32px rgba(0,0,0,0.6), 0 0 0 1px rgba(255,106,0,0.08)'}}>
      <p className="text-[#8FA4C8] text-[10px] font-bold uppercase tracking-[0.15em] mb-1 text-center">{balanceLabel}</p>
