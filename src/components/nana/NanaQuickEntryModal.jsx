            ...
            <div className="space-y-2">
              ...
              {ENTRY_TYPES.map((t) => (
                <button key={t.key} onClick={() => selectType(t.key)}
                  className="w-full flex items-center justify-between bg-[#161616] border border-[#2D2D2D] rounded-xl px-4 py-3 hover:border-[#F97316] hover:bg-[#F97316]/10 transition-all group">
                  <div className="text-left">
                    <p className="text-white text-sm font-medium">{t.label}</p>
                    <p className="text-[#8FA4C8] text-xs">{t.desc}</p>
                  </div>
                  <ChevronRight className="w-4 h-4 text-[#8FA4C8] group-hover:text-[#F97316] transition-colors" />
                </button>
              ))}
            </div>
          )}
          ...
          {step === "form" && (
            <div className="space-y-3">
                ...
                <>
                    ...
                    <div className="flex gap-2">
                      {["expense", "income", "savings"].map((t) => (
                        <button key={t} onClick={() => setField("txType", t)}
                          className={`flex-1 py-2 rounded-xl text-xs font-medium border transition-all ${form.txType === t ? "bg-[#F97316] border-[#F97316] text-white" : "bg-[#161616] border-[#2D2D2D] text-[#8FA4C8] hover:border-[#F97316]"}`}>
                  ...
                  <div>
                    ...
                    <select value={form.category || ""} onChange={(e) => setField("category", e.target.value)}
                      className="w-full bg-[#161616] border border-[#2D2D2D] rounded-xl px-3 py-2 text-white text-xs outline-none focus:border-[#F97316]">
                    ...
                    <div className="grid grid-cols-3 gap-1.5">
                      {INVEST_TYPES.map((t) => (
                        <button key={t.key} onClick={() => setField("investType", t.key)}
                          className={`py-2 rounded-xl text-xs font-medium border transition-all ${form.investType === t.key ? "bg-[#F97316] border-[#F97316] text-white" : "bg-[#161616] border-[#2D2D2D] text-[#8FA4C8] hover:border-[#F97316]"}`}>
                          {t.label}
                        </button>
                      ))}
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-1.5">
                      {DEBT_TYPES.map((t) => (
                        <button key={t.key} onClick={() => setField("debtType", t.key)}
                          className={`py-2 rounded-xl text-xs font-medium border transition-all ${form.debtType === t.key ? "bg-[#F97316] border-[#F97316] text-white" : "bg-[#161616] border-[#2D2D2D] text-[#8FA4C8] hover:border-[#F97316]"}`}>
                          {t.label}
                        </button>
                      ))}
                    </div>
              ...
              <button onClick={handleSend}
                className="w-full mt-2 py-3 bg-[#F97316] hover:bg-[#e05e00] rounded-xl text-white text-sm font-bold transition-colors">
                Kirim ke Nana →
              </button>
            </div>
          )}
