...
export default function BottomSheetSelect({ isOpen, onClose, title, options, onSelect, selectedValue }) {
  return (
    <Drawer.Root open={isOpen} onOpenChange={onClose}>
      <Drawer.Portal>
        <Drawer.Overlay className="fixed inset-0 z-[290] bg-black/40" />
        <Drawer.Content className="fixed bottom-0 left-0 right-0 z-[300] flex flex-col rounded-t-3xl bg-white max-h-[85vh] sm:max-w-md sm:rounded-2xl sm:bottom-auto sm:left-1/2 sm:top-1/2 sm:-translate-x-1/2 sm:-translate-y-1/2 overscroll-contain">
