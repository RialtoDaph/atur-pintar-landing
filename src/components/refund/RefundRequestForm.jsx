  ...
  return (
    <form onSubmit={handleSubmit} className="mt-4 bg-white/[0.04] border border-white/10 rounded-2xl p-4 sm:p-5 space-y-3">
      ...
      <div>
        ...
        <textarea data-refund-field="reason" required className={`${inputCls} min-h-[80px] resize-none`} value={form.reason} onChange={handleChange("reason")} placeholder="Jelaskan kenapa kamu meminta refund..." />
      </div>
      ...
      <button type="submit" disabled={submitting} className="w-full mt-2 bg-[#FF6A00] hover:bg-[#FF6A00]/90 disabled:opacity-50 text-white font-bold text-sm py-3 rounded-xl flex items-center justify-center gap-2">
