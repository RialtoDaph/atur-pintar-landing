...
Deno.serve(async (req) => {
  try {
    ...
    const logErr = (label) => (e) => console.error(`${label} delete failed:`, e);
    const results = {};
    ...
    for (const acc of accounts) {
      ...
      if (!accountsByName[key]) {
        ...
        await base44.entities.Account.delete(acc.id).catch(logErr("Account"));
      }
    }
    ...
    for (const g of goals) {
      ...
      if (!goalsByName[key]) {
        ...
        await base44.entities.SavingsGoal.delete(g.id).catch(logErr("SavingsGoal"));
      }
    }
    ...
    for (const r of reminders) {
      ...
      if (!remindersByTitle[key]) {
        ...
        await base44.entities.Reminder.delete(r.id).catch(logErr("Reminder"));
      }
    }
    ...
    for (const s of subs) {
      ...
      if (!subsByName[key]) {
        ...
        await base44.entities.Subscription.delete(s.id).catch(logErr("Subscription"));
      }
    }
    ...
    for (const a of alerts) {
      ...
      if (a.created_date && a.created_date < sixtyDaysAgo) {
        await base44.entities.Alert.delete(a.id).catch(logErr("Alert"));
        alertsDeleted++;
        continue;
      }
      ...
      if (!alertsByTitle[key]) {
        ...
        if ((a.created_date || "") > (existing.created_date || "")) {
          await base44.entities.Alert.delete(existing.id).catch(logErr("Alert"));
          alertsByTitle[key] = a;
        } else {
          await base44.entities.Alert.delete(a.id).catch(logErr("Alert"));
        }
    ...
    if (profiles.length > 1) {
      ...
      for (const p of sorted.slice(1)) {
        await base44.entities.GamificationProfile.delete(p.id).catch(logErr("GamificationProfile"));
      }
